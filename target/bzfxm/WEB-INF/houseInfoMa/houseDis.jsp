...
<div data-options="region:'north',border:false" style="overflow: hidden" id="publicNorth">
				<ul class="search-group">
					...
					editable:false,valueField:'piItemcode',textField:'piItemname'">
					</li>
                       ...
                       var username=((data[i].applyUserinfo).username==undefined)?"":(data[i].applyUserinfo).username;
                        if(data[i].orderflag=="TRUE"){
                            $('.houseContainer').append(' <li class="houseItem nocheck">' +
                                ...
                                '<p class="houseName"></p>'+
                                '<p class="houseAreaGroup"><input type="checkbox" name="selectRadio" class="selectRadio" value="'+data[i].factmappingId+'" >'+data[i].fHouse+'<span>'+data[i].fConacre+'m²</span></p>' +
                                '</li>')
                        }else{
...
/*    var clickFlag = true;
    $('body').on('click','.houseContainer .nocheck',function(e){
        if(clickFlag == true){
            var sfzh=$.trim($("#sfzh").combogrid('getValue'));
            if(sfzh.length==0){
                $(this).prop("checked",false);
                ...
                if(build.length < perHouseCount){
                    $(this).find('.selectRadio').prop('checked',true);
                    //在数组中，不加入，不在数组中加入
                    if($.inArray(id,build)==-1){
                        build.push(id);
                        clickFlag = true;
                    }
                } else {
                    bank.alertMessage("每人最多可以分配三套房源！")
                    $(this).find('.selectRadio').prop('checked',false);
                    clickFlag = false;
                }
            }
        }
    ...
    $('body').on('dblclick','.houseContainer .nocheck',function(e){
        var id= $(this).find('.houseCode').attr('data-houseId');
        $(this).find('.selectRadio').prop('checked',false);
        if($.inArray(id,build)!=-1){
            var index=bank.getArrayIndex(build,id);  //获取index
            build.splice(index,1);
            maxLength=maxLength+1;
            clickFlag = true;
        }
	...
	//根据身份证号加载申请单和个人信息
	var perHouseCount  = 0;//根据下拉列表，请求用户个人名下的房源数目
    function selectUser(rowIndex, rowData){
		...
		maxLength=3-Number(rowData.numb);
         ...
         /*查询用户名下的房源数目*/
         $.post(
             "<%=basePath %>FactMapping/findFactCountByApplyid"
			 ,{applyid:resultData.aplid},
			 function (data) {
                 perHouseCount=data;
				 $('#msg').html('友情提示：申请人已分配'+ data + '套房源！');
                 perHouseCount = 3 - perHouseCount;
             },
			 "json"
		 );
    ...
    $('#xjy').click(function () {
        $("#ssq").combobox('setValue',0);
        $("#ssq").combobox('readonly',true);
    })
    $('#dbtk').click(function () {
        $("#ssq").combobox('readonly',false);
    })
    $('#jsf').click(function () {
        $("#ssq").combobox('readonly',false);
    })
