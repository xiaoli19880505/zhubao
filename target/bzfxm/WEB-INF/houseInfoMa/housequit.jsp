            ...
            <form method="post" id="form" class="easyui-form">
                <ul class="search-group-add windowInner">
                    ...
                    <li style="width: 380px">
                        <input class="easyui-textbox" id="acremark" name="acremark" style="width: 92%;height: 100px" data-options="label:'备注:',multiline:true">
                    </li>
                </ul>
            </form>
        </div>
...
<script>
   ...
   function backHouse(atype,aplid,apsqlb){
       ...
       $.messager.confirm('操作提示', '您确定放弃吗？', function (r) {
           if (r) {
               clearBankInfo();
    ...
    //根据条件查询
    function query(){
       var ssqname = $.trim($("#ssq").combobox('getText'));
       if(ssqname=='市区'){
           ssqname="";
       }
        $("#dataTable").datagrid('load',{
            ...
            sqbh:$.trim($("#sqbh").val()),
            ssq:ssqname,
            ssj:$.trim($("#ssjd").combobox('getText'))
        });
    }

    function save(){
        var saveRow = $("#dataTable").datagrid("getSelected");
        $('#form').form('submit', {
