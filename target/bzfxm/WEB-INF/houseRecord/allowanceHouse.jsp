	...
	<div class="easyui-panel resPanel" style="width: 100%;height: 98%;overflow: auto">
		<table id="dataTable" class="easyui-datagrid resTable" style="height: 100%;min-height: 300px"
			   data-options="url:'<%=basePath%>apply/selectApply',singleSelect:true,rownumbers:true,pagination:true,border:true,striped:true,
                       pagePosition:'bottom',pageSize:20,nowrap:true,fitColumns:true,queryParams:{applyType:3}">
			<thead>
			<tr>
...
<script>
    ...
    function operate(value,row,index) {
        ...
        var  apLc = row.apLc;
        var cType = "";
        if(row.contractDetail == "" || row.contractDetail == undefined){
            cType = 1;
        ...
        if(cType == 1){
            if(apLc == 4){
                return ' <a class="agreen" onclick="auditOpinion(3,\''+appid+'\')">查看</a><a class="agreen" onclick="infoEntry(\''+appid+'\',\''+cType+'\')">合同信息录入</a>'
            }else if(apLc == 5){
    ...
    function downLoad(appid){
        window.location.href="<%=basePath %>contract/downLoad?apSqlb=3&objId="+appid
    }
    //查看
