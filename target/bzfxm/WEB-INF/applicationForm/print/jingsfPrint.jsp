	...
	<script language="javascript" type="text/javascript">
        function print() {
            ...
            var headerHTML = '<html><head><title>徐州市市区公共租赁住房申请审核表（新就业人员）</title></head><body>'
                ...
                + 'table tr th,table tr td{line-height:20px;height:45px;text-align:center;font-weight:normal;font-size:12px;font-family:宋体;}'
                + '.td_1{width:1%;}'
                + '.td_6{width:6%;}'
                + '.td_10{width:10%;}'
                + '.td_12{width:12%;}'
...
<div id = "contain1" style="margin:0 auto;width:600px;">
	<h1 style="text-align:center;font-family:'宋体';">徐州市市区经济适用住房申请审核表</h1>
	...
	<table style="">
		<tr>
			...
			<td class="td_35" colspan="35">${apply.applyFamilyMembers[0].afmSfzh }</td>
			<td class="td_15" colspan="15">性别</td>
			<td class="td_15" colspan="15">${apply.applyFamilyMembers[0].afmXb }</td>
		</tr>
		<tr>
			<td class="td_10" colspan="10">工作单位</td>
			<td class="td_35" colspan="35">${apply.dwmc }</td>
			<td class="td_15" colspan="15">联系电话</td>
			<td class="td_15" colspan="15">${apply.applyFamilyMembers[0].afmLxdh }</td>
		</tr>
		<tr>
			<td class="td_10" colspan="10" rowspan="2">配偶姓名</td>
		<c:if test="${apply.applyFamilyMembers[1].afmXm!=null }" >
			<td class="td_15" colspan="15" rowspan="2">${apply.applyFamilyMembers[1].afmXm }</td>
			<td class="td_10" colspan="10">身份证号</td>
			<td class="td_35" colspan="35">${apply.applyFamilyMembers[1].afmSfzh }</td>
			<td class="td_15" colspan="15">性别</td>
			<td class="td_15" colspan="15">${apply.applyFamilyMembers[1].afmXb }</td>
		</tr>
		<tr>
			<td class="td_10" colspan="10">工作单位</td>
			<td class="td_35" colspan="35">${apply.applyFamilyMembers[1].afmGzdw }</td>
			<td class="td_15" colspan="15">联系电话</td>
			<td class="td_15" colspan="15">${apply.applyFamilyMembers[1].afmLxdh }</td>
		</c:if>
		<c:if test="${apply.applyFamilyMembers[1].afmXm==null }" >
			<td class="td_15" colspan="15" rowspan="2"></td>
			<td class="td_10" colspan="10">身份证号</td>
		...
		<tr>
			...
			<td class="td_15" colspan="15">${apply.apSqhjnx}</td>
			<td class="td_25" colspan="25">家庭年收入${apply.apJtnsr} 元</td>
			<td class="td_10" colspan="10">家庭人口</td>
			<td class="td_10" colspan="10">${apply.apJtrk}</td>
			<td class="td_30" colspan="30">人均月收入${apply.apRjysr} 元</td>
		</tr>
		<tr>
		...
		<c:forEach items="${apply.applyFamilyHouses }" varStatus="i" var="item" >
			<c:if test="${i.index == 0}" >
				<tr>
					...
					<td class="td_18" colspan="18" rowspan="2">${item.afhRjmj }</td>
				</tr>
			</c:if>
			<c:if test="${i.index == 1}" >
				<tr>
		...
		<!--<tr>
			<td class="td_36" colspan="36"></td>
			<td class="td_18" colspan="18"></td>
			<td class="td_18" colspan="18"></td>
		</tr>-->
		<tr>
			<td class="td_10" colspan="10">现住房性质<br/>（√）</td>
			<td class="td_18" colspan="18">直管公房（<c:if test="${fn:length(apply.applyFamilyHouses)>0 && apply.applyFamilyHouses[0].afhZfxz == '1'}">√</c:if>）</td>
			<td class="td_18" colspan="18">单位公房（<c:if test="${fn:length(apply.applyFamilyHouses)>0 && apply.applyFamilyHouses[0].afhZfxz == '2'}">√</c:if>）</td>
			<td class="td_18" colspan="18">租赁私房（<c:if test="${fn:length(apply.applyFamilyHouses)>0 && apply.applyFamilyHouses[0].afhZfxz == '3'}">√</c:if>）</td>
			<td class="td_18" colspan="18">自有私房（<c:if test="${fn:length(apply.applyFamilyHouses)>0 && apply.applyFamilyHouses[0].afhZfxz == '4'}">√</c:if>）</td>
			<td class="td_18" colspan="18">借住（<c:if test="${fn:length(apply.applyFamilyHouses)>0 && apply.applyFamilyHouses[0].afhZfxz == '5'}">√</c:if>）</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="td_10" colspan="10" rowspan="6">同住<br/>家庭<br/>成员<br/>情况</td>
...
<div id = "contain2" style="margin:0 auto;width:600px;">
	<table style="">
		<tr>
			<td class="td_20" colspan="20">申请人所属办事<br/>处审核意见</td>
		...
		<tr>
			<td class="td_20" colspan="20">区民政部门<br/>审核意见</td>
			<td class="td_80" colspan="80" style="text-align:left;padding-left:10px;">
		...
		<tr>
			<td class="td_20" colspan="20">区住房保障部<br/>门审核意见</td>
			<td class="td_80" colspan="80" style="text-align:left;padding-left:10px;">
