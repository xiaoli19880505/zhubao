...
<head>
	...
	<script language="javascript" type="text/javascript">
        function print() {
            ...
            var headerHTML = '<html><head><title>徐州市市区公共租赁住房申请审核表（新就业人员）</title></head><body>'
                ...
                + 'table tr th,table tr td{line-height:20px;height:30px;text-align:center;font-weight:normal;font-size:12px;font-family:宋体;}'
                + '.td_1{width:1%;}'
                + '.td_6{width:6%;}'
                + '.td_10{width:10%;}'
                + '.td_12{width:12%;}'
	...
	<style type="text/css">
		table,tr,th,td,th,br{margin:0;padding:0;}
		table{width:100%;border-collapse:collapse;}
		table,table tr th, table tr td{border:1px solid #000;}
...
<div id = "contain1" style="margin:0 auto;width:600px;">
	...
	<div style="margin-right:2%;margin-bottom:10px;text-align:right;font:normal 16px '宋体';"> ${applyButie.ssqStr } ${applyButie.ssjStr }街道办事处 ${applyButie.abSqjwh }社区居委会</div>
	<table style="">
		<tr>
		...
		<c:forEach items="${applyButie.applyFamilyMembers }" varStatus="i" var="item" >
			<c:if test="${item.afmXm != null}" >
				...
				<td class="td_10" colspan="10">
					<c:choose>
						<c:when test="${item.afmHyzk==2 or item.afmYsqrgx=='配偶' }">已婚</c:when>
