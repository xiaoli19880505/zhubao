...
%>
<jsp:include page="../../index/header.jsp" flush="true"></jsp:include>
...
<div class="over" style="display: none;">
	...
	<div data-options="region:'center',border:false" id="flowPanel">
		...
		<!--右侧流程图-->
		<div title="流程配置">
			<div class="easyui-panel mb10" title="流程配置">
				<div class="ckprocess">
					<div class="quan">开始</div>
					<!--放置新增节点-->
		...
		<!--右侧流程图--end！-->
		<div class="easyui-panel mb10 " title="流程属性" style="background: #fff;">
				<table id="lcattribute" cellpadding="0" cellspacing="0">
					<tr>
						<td><input class="easyui-textbox" name="flowname" id="flowname" style="width: 90%" data-options="label:'流程名称',validType:['empty','allNumber','specialCharacter']"/></td>
						<td><input class="easyui-textbox" name="flowno" id="flowno" style="width: 90%" data-options="label:'流程编码',validType:['empty','allNumber','specialCharacter']"/></td>
					</tr>
				</table>
		</div>
		<!--右侧数据 end-->
			...
			$('body').on('click','#processbox  .fang',function(e) {
								...
								if (($(this).text() == "节点名称")&& ($("#flowattr").find('#form' + index + '').length == 0)) {
                                    var targetObj = $('<div class="panel panel-htop" style="display: block;">'
                                        ...
                                        + '<td><input name="node_role_id"  class="easyui-combogrid"  style="width:90%" data-options="label:\'接收人\',panelHeight:\'auto\',panelMaxHeight:\'300\',idField:\'dutyid\',textField:\'dutyname\','
                    ...
                    if (isValid){
                        ...
                        if(flag){
                            for ( var i = 0; i < formarray.length; i++) {
                                addJSON[formarray[i]] = {};
                                addJSON[formarray[i]].nodename = $("#form" + formarray[i] + "").find(".nodename").val();
                                addJSON[formarray[i]].nodeno = $("#form" + formarray[i] + "").find(".nodeno").val();
                                addJSON[formarray[i]].node_role_id = $("#form" + formarray[i] + "").find(".easyui-combogrid").combogrid("getValue");
                                addJSON[formarray[i]].desc = $("#form" + formarray[i] + "").find(".desc").val();
                            }
