...
<body>
	    ...
	    <div class="mui-content mui-scroll-wrapper">
	      <div class="mui-scroll">
	        ...
	        <ul class="mui-table-view ullist">
				<li class="mui-table-view-cell">申请类型<span style="float: right">
					<c:choose>
					    <c:when test="${appove.aptype=='ns2'}">公共租赁住房租赁补贴</c:when>
						<c:when test="${appove.aptype=='ns3'}">公共租赁住房（低保、特困家庭）</c:when>
						<c:when test="${appove.aptype=='ns4'}">公共租赁住房（外来务工人员）</c:when>
						<c:when test="${appove.aptype=='ns5'}">公共租赁住房（新就业人员）</c:when>
				   </c:choose></span></li>
				...
				<li class="mui-table-view-cell">身份证号<span style="float: right">${appove.applyUserinfo.sfzh}</span></li>
	...
	<script>
        ...
        mui(".mui-table-view-cell").on('tap', '.toedit', function(e) {
            weixin.openWindow('toView','<%=basePath %>appApply/appAuditUpdate?applyType=${appove.atype}&applyId=${appove.aplid}');
        });
		//修改
        mui(".mui-table-view-cell").on('tap', '.toview', function(e) {
            weixin.openWindow('toEdit','<%=basePath %>appApply/appAuditViewApprove?applyType=${appove.atype}&applyId=${appove.aplid}');
        });
        //返回上一页
