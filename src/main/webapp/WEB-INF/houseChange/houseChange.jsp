...
<div data-options="region:'north',border:false" style="overflow: hidden" id="publicNorth">
				<ul class="search-group">
					...
					editable:false,valueField:'piItemcode',textField:'piItemname',onSelect:selectSsq">
					</li>
					<li style="width: 600px">
						<label style="width:80px;display: inline-block">房源类型:</label>
						<input type="radio" name="houseType" value="1" checked id="1"><label class="lispan" for="1" id="jsf">经适房</label><span class="lispace"></span>
						<input type="radio" name="houseType" value="2" id="2"><label class="lispan" for="2" id="dbtk">公租房(低保特困)</label><span class="lispace"></span>
						<input type="radio" name="houseType" value="3" id="3"><label class="lispan" for="3" id="xjy">公租房(外来务工、新就业)</label><span class="lispace"></span>
					</li>
					<li class="query-btn" >
	            ...
	            <ul class="search-group">
					<li style="width: 450px"><input class="easyui-combogrid" id="sfzh" data-options="label:'申请人',panelHeight:'auto',panelMaxHeight:'300',idField:'apluserid',textField:'username',
					pagination:true,pageSize:10,events:{keyup:queryUser},onSelect:selectUser,columns: [[
						{field:'username',title:'姓名',width:50,align:'center'},{field:'sfzh',title:'身份证号',width:120,align:'center'}]],fitColumns: true" style="width: 92%" >
					</li>
					<li class="query-btn" >
						<a class="icon iconfont icon-shoucang distribute" onclick="distribute()"><i>分配</i></a>
					</li>
					<li class="query-btn" style="width: 120px!important;">
						<a class="icon iconfont icon-shoucang distribute" onclick="changeRecord()"><i>申请人分房记录</i></a>
					</li>
				</ul>
</div>
...
<script>
	...
	function loadApplyer(ssqId,radioId,uname) {
        ...
        $("#sfzh").combogrid("grid").datagrid({
            url:'<%=basePath %>appove/findChangePersons',
            queryParams:{
                ssq: ssqId,
                aptype: radioId,
                uname:uname
    ...
    function distribute(){
        if(isClick){
            if($.trim($("#project").combobox('getValue')).length==0){
             ...
             var radioId=$.trim($('input[name="houseType"]:checked').val());
                $.messager.confirm('操作提示', '您确定调房吗？', function (r) {
                    if (r) {
                        $.ajax({
                            url:'<%=basePath %>sourceHouse/updateHouseReset',
                            type:'post',
                            data:{
    ...
    function changeRecord(){
        var sfzh= $("#sfzh").combogrid('getValue');
        if($.trim(sfzh).length==0){
            bank.alertMessage("请选择申请人！")
		}else{
            var applyid=resultData.aplid;
			bank.biography().setParams({row:applyid,title:"changeRecord"});
			bank.showWindow('<%=basePath %>path/toFenPeiTable', "申请人分房记录", 600, 500, true);
		}
    }
    //为街道和街区默认赋值
    $(function () {
