...
<script>
	$(function () {
        var data=bank.biography().getParams('moduleEdit');
        $("#changeForm").form('load',data.row);
    });

    function  saveEdit() {//保存
        var data=bank.biography().getParams('moduleEdit');
        bank.ajaxForm('#changeForm','<%=basePath %>menuInfo/updateMenuInfo',{
            moduleid:data.row.meId,
			faid:data.row.meFaid},function(data){
            if(data=="修改成功"){
                setTimeout(function () {
