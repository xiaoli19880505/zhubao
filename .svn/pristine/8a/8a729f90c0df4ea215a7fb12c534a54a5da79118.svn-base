	...
	<div class="easyui-panel" style="width: 100%;overflow: hidden">
		...
		<form id="queryForm">
			<ul class="search-group">
				<li><input type="radio" name="all" value="all" checked="true"><label class="lispan">全部审核</label>
		...
		<%--<div class="abtn-group">
			<a href="javascript:void(0)" class="icon iconfont icon-20daochu green" onclick="downloadExcel()"><i>导出EXCEL</i></a>
		</div>--%>
	</div>
</div>
<div data-options="region:'center',border:false">
	<div class="easyui-panel resPanel" style="width: 100%;height: 98%;overflow: auto">
		<table id="dataTable" class="easyui-datagrid resTable" style="height: 100%;min-height: 300px"
			   data-options="url:'<%=basePath %>task/listPerToHandleTask',singleSelect:true,rownumbers:true,pagination:true,border:true,striped:true,
...
<script>
    ...
    function auditStart(apptype,appid,processInstanceId){
        var a = $('<a href="<%=basePath %>task/toApprove?applyType='+apptype+'&applyId='+appid+'&processInstanceId='+processInstanceId+'&chengxin=feicheng" target="_blank"></a>')[0];
	...
	function auditProcess(appid,processInstanceId){
        ...
        bank.showWindow("<%=basePath %>task/toApproveProcess","审核进度",740, 300, true);
	}
    ...
    function downloadExcel() {
        ...
        var applyType=$.trim($('.selectType').find('input[name="typeCode"]').val());
        var applyUserJd =$.trim($("#ssjd").combobox('getValue'));
        var applyUserSsq =$.trim($("#ssq").combobox('getValue'));
        window.location.href="<%=basePath%>/task/downloadDsh?applyType="+applyType+"&applyUserJd="+applyUserJd+"&all="+all+"&applyUserSsq="+applyUserSsq;
    }
