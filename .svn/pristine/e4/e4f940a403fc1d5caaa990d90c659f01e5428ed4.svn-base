 ...
 * @desc 工具类：小写金额转换成大写金额
 */
public class CnNumberUtils {

    private final static Logger logger = Logger.getLogger(CnNumberUtils.class); //阿拉伯数字对应大写中文数字
    private final static String[] CN_NUMERALS =
            {"零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖"}; //低单位
    private final static String[] CN_NUM_UNIT = {"", "拾", "佰", "仟"}; //万元以上高单位
    private final static String[] CN_NUM_HIGHT_UNIT = {"", "万", "亿", "万亿"}; //小数位单位
    ...
    public static String toUppercase(String num){
        ...
        for(int i=0; i<intLength; i++){
            ...
            if(!isZero(curCnNum)){
                ...
                String heightUnit = CN_NUM_HIGHT_UNIT[hui]; //如果剩余没有低金额单位或者后续都为零则加上高单位
                if(ui == 0 || isZeroInResidue(chars, i, ui)){
                    strBuilder.append(heightUnit); //万单位以后的千单位如果不能为零则跳过零
        ...
        clearZero(0, strBuilder); //小数位转换
        strBuilder.append("圆");
        if(strNumArrayLength == 1 || strNumArray[1].matches("^[0]*$")){
            strBuilder.append("整"); return strBuilder.toString();
        }
