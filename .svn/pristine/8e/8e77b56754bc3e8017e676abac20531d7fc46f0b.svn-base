...
<script>
    function operate(value,row,index) {
        ...
        if(cType == 1){
            if(affLc == 4){
                return ' <a class="agreen" onclick="auditOpinion(4,\''+appid+'\')">查看</a><a class="agreen" onclick="infoEntry(\''+appid+'\',\''+cType+'\')">合同信息录入</a>'
            }else if(affLc == 5){
	...
	function infoEntry(appid,cType) {
        $.ajax({
            ...
            success:function (data) {
                ...
                bank.biography().setParams({row:data,title:'informationWLWG'});
