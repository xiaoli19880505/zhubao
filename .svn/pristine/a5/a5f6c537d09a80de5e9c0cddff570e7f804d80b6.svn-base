...
public class FtpUtils {
    ...
    public static String FTPUrl = "http://" + PropertiesUtil.getFTPProperties("hostname")
            +  ":" + Integer.parseInt(PropertiesUtil.getFTPProperties("httpport")) + "/";//80可以省略不写

    /*static {
        System.out.println("初始化结构代码块FtpUtil……");
        initFtpClient();
    }*/
    /**
     * 初始化ftp服务器
    ...
    public static void initFtpClient() {
        ...
        try {
            String hostname = PropertiesUtil.getFTPProperties("hostname");//链接地址
            Integer port = Integer.parseInt(PropertiesUtil.getFTPProperties("port"));//端口
            String username = PropertiesUtil.getFTPProperties("username");//链接用户
            String password = PropertiesUtil.getFTPProperties("password");//密码
...
//            ftpClient.makeDirectory(fileName);
            ...
            ftpClient.changeWorkingDirectory(pathname);//上传目录
            ftpClient.setBufferSize(1024);
//            ftpClient.setControlEncoding("GBK");
            ftpClient.setFileType(ftpClient.BINARY_FILE_TYPE);

//            FileInputStream  fis = (FileInputStream)inputStream;
//            System.out.println(fileName.getBytes("utf-8"));
            Boolean b = ftpClient.storeFile(
                    new String(fileName.getBytes("UTF-8"), "ISO-8859-1"),//,"iso-8859-1"
                    inputStream);
     ...
     * @param filename  下载命名的文件名称 *
     * @return */
    public static boolean downloadFile(String pathname, String filename, HttpServletResponse response){
        boolean flag = false;
        ...
        try {
            ...
            boolean status = ftpClient.retrieveFile(pathname, out);
