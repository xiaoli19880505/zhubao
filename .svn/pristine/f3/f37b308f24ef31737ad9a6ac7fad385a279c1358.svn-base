    ...
    */
    public static Jedis getJedis() {
       // return jedisPool.getResource();
        Jedis  redis = new Jedis ("127.0.0.1",6379);//����redis
		redis.auth("redis");//��֤����   
		return redis;
    }
    ...
    */
    public static void sadd(String key, String... members) {
        ...
        try {
            ...
            logger.info("sadd：向缓存中添加数据，key：{} value：{}");
        } catch (Exception e) {
            logger.error("sadd：向缓存中添加数据时出现异常，key：{} value：{}");
        } finally {
            //保证jedis被关闭（归还给连接池）
            closeQuietly(jedis);
        }
    ...
    public static Set<String> smembersAndDel(String key) {
        ...
        try {
            ...
            if (result == null || result.isEmpty()) {
                ...
                logger.debug("smembersAndDel：从缓存中取出数据同时删除了key，key：{}，value：{}");
            }
    ...
    */
    public static void setex(String key, int expire, String value) {
        ...
        try {
            ...
            logger.debug("setex：向缓存中添加数据，key：{}，value：{}，过期时间：{}秒");
        } catch (Exception e) {
            logger.error("setex：向缓存中添加数据时出现异常，key：{}，value：{}，过期时间：{}秒");
        } finally {
            //保证jedis被关闭（归还给连接池）
            closeQuietly(jedis);
        }
    ...
    */
    public static String get(String key) {
        ...
        try {
            jedis = getJedis();
            value = jedis.get(key);
            logger.debug("get：从缓存中获取数据，key：{}，value：{}");
        } catch (Exception e) {
            logger.error("get：从缓存中获取数据时出现异常，key：{}，value：{}");
        } finally {
            //保证jedis被关闭（归还给连接池）
            closeQuietly(jedis);
        }
    ...
    */
    public static void set(String key, String value){
        ...
        try {
            jedis = getJedis();
            jedis.set(key, value);
            logger.debug("set：向缓存中添加数据，key：{}，value：{}");
        } catch (Exception e) {
            logger.error("set：向缓存中添加数据时出现异常，key：{}，value：{}");
        } finally {
            //保证jedis被关闭（归还给连接池）
            closeQuietly(jedis);
        }
    ...
    */
    public static boolean isExist(String key){
         ...
         try {
             ...
             if(exist){
             	logger.debug("根据此key:{}，可以在缓存中找到对应的value");
             	return true;
             } else {
            	logger.debug("根据此key:{}，并没有在缓存中找到对应的value");
            	return false;
             }
         } catch (Exception e) {
             logger.error("get：从缓存中获取数据时出现异常，key：{}");
    ...
    */
    public static void del(String keyPattern) {
        ...
        try {
            ...
            logger.debug("要删除的key为：{}");

            if (keys != null && keys.size() > 0) {
                String[] arr = new String[keys.size()];
                jedis.del(keys.toArray(arr));
            }
            logger.debug("del：从缓存中删除数据，keyPattern：{}");
        } catch (Exception e) {
            logger.error("del：从缓存中删除数据时出现异常，keyPattern：{}");
        } finally {
            //保证jedis被关闭（归还给连接池）
            closeQuietly(jedis);
        }
